...
+ (id)nextKeyboardKey {
    NSArray *nextKeyboardAccessibilityLabels =
    @[
      @"Следующая клавиатура",
      @"Next keyboard"
      ];
    
    for (id accessibilityLabel in nextKeyboardAccessibilityLabels) {
        id nextKeyboardKey = [GREYKeyboard grey_findKeyForCharacter:accessibilityLabel];
...
+ (BOOL)typeCharacterAsString:(NSString *)characterAsString
    ...
    if (!key) {
      ...
      if (!key && trySwitchingLaunguage) {
        id changeLanguageKey = [self nextKeyboardKey];
        
        // TODO: Pass proper accessibilityLabel (from [self nextKeyboardKey])
